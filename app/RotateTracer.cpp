...
RotateTracer::RotateTracer(Walker* walker, int direction, int angleDeg, int pwm)
    : mWalker(walker),
      ...
      mTargetTurnWdeg(static_cast<int>(std::abs(angleDeg) * WHEEL_DEG_PER_BODY_DEG)),
      mPwm(std::abs(pwm)),
      mStartLeftCount(0),
      mStartRightCount(0)
{
    ...
    switch(mState) {
        ...
        case WAITING_FOR_START:
            if(mStarterList.empty()) {
                ...
                mState = WALKING;
                LOGI("[ROTATE] start: direction=%s angle=%d wheelDeg pwm=%d\n",
                     mDirection > 0 ? "RIGHT" : "LEFT", mTargetTurnWdeg, mPwm);
                return;
            }
            ...
            break;
        case WALKING:
            if(std::abs(getTurnWdeg()) >= mTargetTurnWdeg) {
                mWalker->brake();
                mState = TERMINATED;
